...
fn test_optimization_worker_specs_route_primal_workers_verdict_incapable() {
    ...
    // The unicost-covering arm sits DIRECTLY BEHIND the five complete
    // baselines, on the same rationale as `wbo-sls-opt` on the WBO route: its
    // gate is one structural O(nonzeros) pass, so off-family it declines in
    // microseconds and its core is immediately backfilled, while on-family it
    // is the only arm that can close the last unit of a covering objective.
    assert_eq!(
        labels[5], "card-descent-opt",
        "the covering arm must sit directly behind the five complete baselines: {labels:?}"
    );
    assert_eq!(
        labels.len(),
        13,
        "5 complete baselines + card-descent + P6/P7 + the 4 diversified arms + P12 DDFW: {labels:?}"
    );
...
fn test_optimization_worker_specs_wbo_route_appends_wbo_sls_arm() {
    ...
    assert_eq!(
        labels[6], "card-descent-opt",
        "the covering arm stays directly behind the WBO high-cap arm: {labels:?}"
    );
    assert_eq!(
        labels.len(),
        14,
...
/// The `card-descent-opt` worker body: on a UNICOST COVERING instance it
/// streams verified feasible incumbents and returns the best as
/// `Satisfiable` — NEVER a verdict — and on anything outside the covering
/// fragment it declines with no incumbent at all, which is what makes its
/// high spec position free for off-family instances.
#[test]
fn test_card_descent_worker_covers_then_declines() {
    // 5-cycle dominating set: unicost objective, monotone `>=` rows, optimum 2.
    let covering = parse_opb(
        "* #variable= 5 #constraint= 5\nmin: +1 x1 +1 x2 +1 x3 +1 x4 +1 x5 ;\n\
+1 x1 +1 x2 +1 x5 >= 1 ;\n+1 x2 +1 x1 +1 x3 >= 1 ;\n+1 x3 +1 x2 +1 x4 >= 1 ;\n\
+1 x4 +1 x3 +1 x5 >= 1 ;\n+1 x5 +1 x4 +1 x1 >= 1 ;\n",
    )
    .expect("parse should succeed");
    let objective = covering.objective.clone().expect("has objective");
    let term_flag = AtomicBool::new(false);
    let mut streamed: Vec<(i128, Vec<bool>)> = Vec::new();
    let solution = solve_optimization_card_descent(
        &covering,
        &objective,
        Some(Duration::from_secs(5)),
        Instant::now(),
        &term_flag,
        &mut |value, model| streamed.push((value, model.to_vec())),
    );
    assert_eq!(
        solution.status,
        PbStatus::Satisfiable,
        "a primal arm reports Satisfiable, never a verdict"
    );
    assert!(verify_all_constraints(
    ...
    assert!(
        !streamed.is_empty(),
        "the covering arm must stream incumbents"
    );
    for (value, model) in &streamed {
        assert!(verify_all_constraints(&covering.constraints, model));
        assert_eq!(eval_objective(&objective, model), *value);
    }

    // Off-family: a WEIGHTED objective is outside the gate, so the arm must
    // decline outright (no incumbent, no verdict) rather than burn its core.
    let weighted = parse_opb(
        "* #variable= 3 #constraint= 1\nmin: +2 x1 +3 x2 +5 x3 ;\n+1 x1 +1 x2 +1 x3 >= 2 ;\n",
    )
    .expect("parse should succeed");
    let weighted_objective = weighted.objective.clone().expect("has objective");
    let mut off_family: Vec<i128> = Vec::new();
    let declined = solve_optimization_card_descent(
        &weighted,
        &weighted_objective,
        Some(Duration::from_secs(5)),
        Instant::now(),
        &term_flag,
        &mut |value, _| off_family.push(value),
    );
    assert_eq!(declined.status, PbStatus::Unknown);
    assert!(
        off_family.is_empty(),
        "a declined arm must not stream anything: {off_family:?}"
    );
}
...
/// optimum, and every streamed incumbent is a real witness. With a tight
/// budget of 8 the diversified arms are exactly the specs dropped first
/// (take-first-8 excludes all of them), so core budgets <= 8 keep the
/// diversified arms out of the up-front spawn set.
#[test]
fn test_parallel_optimization_all_diversified_primal_workers_sound() {
    ...
    // UP FRONT, and the priority order puts the diversified arms LAST, so none
    // of them is in the up-front spawn set (they can only enter later via
    // freed-slot backfill, one core at a time, never above the budget).
    //
    // The unicost-covering arm (`card-descent-opt`, index 5) DOES sit inside
    // the budget-7 set, and displaces `sls-primal-opt` out of it — deliberate
    // and bounded: the covering gate is one structural O(nonzeros) pass, so on
    // any instance the arm cannot help (like this weighted-objective fixture)
    // it finishes in microseconds and the coordinator backfills the freed core
    // with `sls-primal-opt` immediately. The diversified arms stay strictly
    // behind BOTH, which is the property that matters for the tight budget.
    let profile = InstanceProfile::from_instance(&instance);
    let specs = optimization_worker_specs(&profile, OptimizationPortfolioRoute::Standard);
    let diversified = [
        "sls-restarts-opt",
    ...
    assert!(
        ...
        "core budget 8 must not spawn any diversified arm"
    );
    assert!(
        specs
            .iter()
...
struct InvalidExternalUpgrade;

impl LinearOptimumUpgrade for InvalidExternalUpgrade {
    fn eligible(&self, _instance: &PbInstance, _objective: &PbObjective) -> bool {
        true
    }
    ...
    fn try_optimum_upgrade(
        ...
        _incumbent: Option<(&[bool], i128)>,
        _budget: Duration,
        on_improve: &mut dyn FnMut(i128, &[bool]),
    ) -> Option<LinearOptimumUpgradeResult> {
        on_improve(0, &[false, false]);
        Some(LinearOptimumUpgradeResult {
            assignment: vec![false, false],
            value: 0,
        })
    }
...
#[test]
fn external_upgrade_boundary_rejects_invalid_result_and_stream() {
    let instance = parse_opb(
        "* #variable= 2 #constraint= 1\n\
         min: +2 x1 +2 x2 ;\n\
         +2 x1 +2 x2 >= 3 ;\n",
    )
    .expect("parse LP-gap fixture");
    let objective = instance.objective.as_ref().expect("objective");
    let incumbent = [true, true];
    let mut streamed = Vec::new();
    let result = run_linear_optimum_upgrade(
        &InvalidExternalUpgrade,
        &instance,
        objective,
        Some((&incumbent, 4)),
        Duration::from_secs(1),
