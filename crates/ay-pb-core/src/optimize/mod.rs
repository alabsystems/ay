...
pub(crate) mod single_row_closure;
pub(crate) mod sls;
#[cfg(test)]
mod sls_sweep;
...
impl<'a> OptimizationEngine<'a> {
    ...
    fn build_persistent_upper_bound_cnf(
        ...
        if weighted_literals.len() < PERSISTENT_BOUND_MIN_TERMS {
            // FORGONE COST: literals the persistent-bound lane declines, every call.
            ay_core::forgone::charge(
                ay_core::forgone::PB_PERSISTENT_BOUND,
                weighted_literals.len() as u64,
            );
            return Ok(None);
        }
