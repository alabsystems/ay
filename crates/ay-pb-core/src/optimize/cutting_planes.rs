...
/// Canonical identity of an emitted cut, used to suppress duplicates across
/// separation rounds. Two cuts with the same key are the same LP row.
pub(crate) type CutKey = (Vec<(u32, bool, i128)>, i128);

/// Canonical key of a cut for duplicate suppression. Terms are sorted so the key
/// does not depend on the order a generator happened to emit them in. Non-linear
/// (product) terms are dropped from the key, which can only make two cuts look
/// EQUAL when they differ — every generator here emits linear rows only, so that
/// case does not arise, and a wrongly-suppressed cut costs tightness, not
/// soundness.
pub(crate) fn cut_key(cut: &PbConstraint) -> CutKey {
    let mut terms: Vec<(u32, bool, i128)> = cut
        .terms
        .iter()
        .filter_map(|t| {
            ...
            Some((lit.var, lit.negated, t.coeff))
        })
        .collect();
    terms.sort_unstable();
    (terms, cut.rhs)
...
pub(crate) fn lit_value(lit: PbLit, x: &FractionalPoint) -> Option<num_rational::BigRational> {
    use num_traits::One;
