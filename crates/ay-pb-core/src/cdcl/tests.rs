...
/// THE ABORT-SEMANTICS CONTRACT — CONFLICT-CAP ARM.
///
/// `solve_with_assumptions_conflict_capped` is safe because of the SHAPE of its
/// implementation: the cap is a stop closure, so it adds no new `Unsatisfiable`
/// return site and inherits the `Unknown`-on-abort behaviour of every existing
/// path. That is a claim about how the function is written, and a future
/// refactor could satisfy the same signature while violating it — a hand-rolled
/// counter inside the search loop with its own return arm would look correct and
/// would turn every exhausted budget into a free `+1` on the lower bound. So the
/// property is pinned directly on the capped entry point instead of being
/// inherited by argument from the interruptible one.
///
/// Uses pigeonhole under assumptions so the UNSAT genuinely requires SEARCH: the
/// instance is `n` pigeons into `n` holes (satisfiable), and the assumptions
/// forbid the last hole, leaving `n` pigeons and `n-1` holes. Random covering
/// rows would be useless here — they are refuted during assumption propagation,
/// so no search conflict ever occurs and the cap would never bind.
///
/// Asserts:
///   1. a capped call that gave up returns `Unknown`, never `Unsatisfiable`;
///   2. any core it DOES return is genuinely unsatisfiable under an independent
///      UNCAPPED solve — including the empty core, which is a claim about the
///      root and is checked the same way; and
///   3. a cap large enough never to bind reproduces the uncapped verdict, i.e.
///      a cap changes EFFORT, never ANSWERS.
#[test]
fn conflict_capped_assumption_solve_never_reports_a_bogus_core() {
    let mut capped_unknowns = 0usize;
    let mut capped_cores = 0usize;
    ...
    for n in 3..=5usize {
        ...
        for p in 0..n {
            constraints.push(ge_constraint(
                (0..n).map(|h| linear_term(1, lit(var(p, h)))).collect(),
                1,
            ));
        }
        // Every hole holds at most one pigeon, written as a covering row over the
        // complements: `sum_p ~x[p][h] >= n - 1`.
        for h in 0..n {
            constraints.push(ge_constraint(
                (0..n).map(|p| linear_term(1, not(var(p, h)))).collect(),
                (n - 1) as i128,
            ));
        }
        let instance = PbInstance {
        ...
        // Forbid the last hole: n pigeons into n-1 holes is UNSAT, and proving it
        // takes real conflicts rather than a single propagation.
        let assumptions: Vec<PbLit> = (0..n).map(|p| not(var(p, n - 1))).collect();
        ...
        for cap in 0..24u64 {
            let mut solver = PbCdclSolver::new(&instance);
            let result = solver.solve_with_assumptions_conflict_capped(&assumptions, cap, || false);

            match result {
                PbCdclAssumptionResult::Unsatisfiable { core } => {
                    capped_cores += 1;
                    // PROPERTY 2. An independent, uncapped solver must agree the
                    // core is unsatisfiable. Applies verbatim to the empty core:
                    // that is a statement about the root, and re-solving with no
                    // assumptions is exactly the right way to check it.
                    let mut check = PbCdclSolver::new(&instance);
                    let verdict = check.solve_with_assumptions(&core);
                    assert!(
                        matches!(verdict, PbCdclAssumptionResult::Unsatisfiable { .. }),
                        "n={n} cap={cap}: reported core {core:?} is NOT unsatisfiable on an \
                         independent re-solve — core-guided search would charge its weight to \
                         the lower bound and over-count"
                    );
                }
                PbCdclAssumptionResult::Unknown => capped_unknowns += 1,
                PbCdclAssumptionResult::Satisfiable(model) => {
                    // PROPERTY 1, contrapositive: a cap must never turn an UNSAT
                    // query into a SAT one either. If it claims a model, the
                    // model must be real.
                    assert!(
                        !model.is_empty(),
                        "n={n} cap={cap}: capped solve claimed SAT on an assumption set that \
                         is unsatisfiable"
                    );
                }
                PbCdclAssumptionResult::Unsupported => {}
            }
        }

        // PROPERTY 3. A cap far above what the proof needs must not perturb the
        // verdict at all.
        let mut generous = PbCdclSolver::new(&instance);
        let capped =
            generous.solve_with_assumptions_conflict_capped(&assumptions, u64::MAX, || false);
        assert_eq!(
            std::mem::discriminant(&capped),
            std::mem::discriminant(&uncapped),
            "n={n}: an unbindable cap changed the verdict — the cap is altering answers, not \
             just effort"
        );
    }

    // The sweep proves nothing if the cap never actually bound, or if it bound so
    // hard that no core ever came back. Assert both directions were exercised.
    assert!(
        capped_unknowns > 0,
        "no capped call ever gave up — the caps never bound, so this test is vacuous"
    );
    assert!(
        capped_cores > 0,
