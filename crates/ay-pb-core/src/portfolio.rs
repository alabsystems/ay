...
/// A candidate optimum returned by an out-of-core linear optimization engine.
///
/// This is deliberately data-only. The portfolio independently rechecks the
/// assignment and objective before it may upgrade a public verdict.
#[doc(hidden)]
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LinearOptimumUpgradeResult {
    pub assignment: Vec<bool>,
    pub value: i128,
}

/// Per-call seam for a leaf package to contribute a linear optimum prover.
///
/// The core PB package has no dependency on the implementing engine. Callers
/// that do not explicitly pass an implementation take the ordinary PB-only
/// route, which also prevents an engine that embeds this core from recursively
/// routing back into itself.
#[doc(hidden)]
pub trait LinearOptimumUpgrade: Send + Sync {
    /// Cheap, side-effect-free admission check used to reserve portfolio time.
    fn eligible(&self, instance: &PbInstance, objective: &PbObjective) -> bool;

    /// Try to prove an optimum within `budget`.
    ///
    /// Implementations may report feasible improvements through `on_improve`,
    /// but the core wraps that callback with its own exact PB verification.
    fn try_optimum_upgrade(
        &self,
        instance: &PbInstance,
    ...
    ) -> Option<LinearOptimumUpgradeResult>;
}

pub(crate) fn run_linear_optimum_upgrade(
    upgrade: &dyn LinearOptimumUpgrade,
    instance: &PbInstance,
    objective: &PbObjective,
    incumbent: Option<(&[bool], i128)>,
    budget: Duration,
    ...
    let incumbent_value = incumbent.map(|(_, value)| value);
    let mut verified_improve = |value: i128, assignment: &[bool]| {
        if assignment.len() == instance.num_vars as usize
            && incumbent_value.is_none_or(|incumbent| value < incumbent)
    ...
    let candidate = upgrade.try_optimum_upgrade(
        instance,
        objective,
        incumbent,
        budget,
        &mut verified_improve,
    )?;
    if candidate.assignment.len() != instance.num_vars as usize
        || incumbent_value.is_some_and(|incumbent| candidate.value > incumbent)
        || !verify_all_constraints(&instance.constraints, &candidate.assignment)
        || eval_objective(objective, &candidate.assignment) != candidate.value
    {
        return None;
    }
    Some(candidate)
...
/// PB optimization with one explicitly supplied external optimum prover.
///
/// The injection is per call rather than process-global, so an embedding engine
/// can always use [`solve_optimization_portfolio`] without routing back into
/// itself.
#[doc(hidden)]
pub fn solve_optimization_portfolio_with_upgrade(
    instance: &PbInstance,
    objective: &PbObjective,
    timeout_dur: Option<Duration>,
    start: Instant,
    term_flag: &AtomicBool,
    on_improve: &mut dyn FnMut(i128, &[bool]),
    upgrade: &'static dyn LinearOptimumUpgrade,
) -> PbSolution {
    solve_optimization_portfolio_with_timings_and_upgrade(
        instance,
        objective,
        timeout_dur,
        start,
...
pub fn solve_optimization_portfolio_with_timings(
    ...
    solve_optimization_portfolio_with_timings_impl(
        instance,
        objective,
        timeout_dur,
        start,
...
/// Timing-returning counterpart of
/// [`solve_optimization_portfolio_with_upgrade`].
#[doc(hidden)]
pub fn solve_optimization_portfolio_with_timings_and_upgrade(
    instance: &PbInstance,
    objective: &PbObjective,
    timeout_dur: Option<Duration>,
    start: Instant,
...
fn solve_optimization_portfolio_with_timings_impl(
    instance: &PbInstance,
    objective: &PbObjective,
    timeout_dur: Option<Duration>,
    start: Instant,
    term_flag: &AtomicBool,
    on_improve: &mut dyn FnMut(i128, &[bool]),
    optimum_upgrade: Option<&'static dyn LinearOptimumUpgrade>,
) -> PbPortfolioOutcome {
    ...
    // Optional leaf-supplied linear optimum prover. The core deliberately knows
    // neither its implementation nor its dependency graph; the public PB-only
    // entry points pass `None`, while the ay-pb leaf injects ay-milp explicitly.
    // Every returned point and streamed incumbent is rechecked below against the
    // original PB instance before it can affect a verdict or anytime output.
    let external_upgrade_eligible = linear_opt
        && instance.num_vars > 0
        && optimum_upgrade.is_some_and(|upgrade| upgrade.eligible(instance, objective));
    let inner_timeout = match timeout_dur {
        ...
        Some(d) if external_upgrade_eligible => {
    ...
    // PRE-SOLVE external engine phase. The engine proves LP-gap instances the rest
    ...
    // adopted verdict is independently re-verified by the core boundary.
    if external_upgrade_eligible && !term_flag.load(Ordering::Relaxed) {
        ...
        if !pre_slice.is_zero() {
            let phase_start = Instant::now();
            let res = run_linear_optimum_upgrade(
                optimum_upgrade.expect("eligible external upgrade"),
                instance,
                objective,
                None,
                pre_slice,
            ...
            if let Some(candidate) = res {
                let solution = PbSolution {
                    ...
                    objective: Some(candidate.value),
                };
                let timings = PbPortfolioPhaseTimings::default();
                return portfolio_outcome(solution, timings, phase_start);
            }
        ...
        && !term_flag.load(Ordering::Relaxed)
    {
        ...
        let remaining = timeout_dur
            ...
            .unwrap_or_else(|| Duration::from_millis(MILP_UPGRADE_RESERVE_MS));
        if !remaining.is_zero() {
            if let Some(candidate) = run_linear_optimum_upgrade(
                optimum_upgrade.expect("eligible external upgrade"),
                instance,
                objective,
                ...
                outcome.solution.status = PbStatus::OptimumFound;
                outcome.solution.assignment = candidate.assignment;
                outcome.solution.objective = Some(candidate.value);
            }
        }
    }
...
fn optimization_worker_specs(
    ...
    optimization_worker_specs_with_upgrade(profile, route, None)
}

fn optimization_worker_specs_with_upgrade(
    profile: &InstanceProfile,
    route: OptimizationPortfolioRoute,
    optimum_upgrade: Option<&'static dyn LinearOptimumUpgrade>,
) -> Vec<OptimizationWorkerSpec> {
    ...
    specs.push(OptimizationWorkerSpec {
        ...
        run: OptimizationWorkerKind::Complete(Box::new(
            move |instance, objective, timeout_dur, start, term_flag, on_improve, _bounds| {
                match optimum_upgrade {
                    Some(upgrade) => solve_optimization_portfolio_with_upgrade(
                        instance,
                        objective,
                        timeout_dur,
                        start,
                    ...
                    None => solve_optimization_portfolio(
                        instance,
                        objective,
                        timeout_dur,
                        start,
    ...
    // [PC] Fixed-cardinality violation descent (`optimize::card_descent`): the
    // NuMVC/RWLS-shaped primal for UNICOST COVERING instances (dominating set,
    // set cover, vertex cover). It holds `|S|` FIXED at `best - 1`, drops the
    // objective from the cost function entirely, and closes the last unit of
    // the objective by remove+add SWAPS under dynamic row weights — the move
    // the P7-P12 SLS arms structurally cannot make, because they minimize a
    // penalty BLEND in which a feasible `k+1` point and an infeasible `k`
    // point are comparable and the search stalls between them.
    //
    // PLACED AHEAD OF THE LINEAR PRIMAL ARMS, on the same rationale as
    // `wbo-sls-opt` above: its applicability gate is STRUCTURAL and decides in
    // one O(nonzeros) pass (`card_descent::build_cover_view`), so on anything
    // outside the unicost-covering fragment it declines in microseconds and
    // the coordinator immediately backfills its core with the arm that would
    // otherwise have held the slot — the position costs off-family instances
    // nothing. On-family it is the strongest primal by a wide margin
    // (measured: `domset_v500_e2000_w30_mw19_19` 177 -> 176 in under a second,
    // where the whole rest of the portfolio flatlines at 177 for 300s).
    if profile.is_linear && profile.has_objective {
        specs.push(OptimizationWorkerSpec {
            label: "card-descent-opt",
            run: OptimizationWorkerKind::Primal(Box::new(
                |instance, objective, timeout_dur, start, term_flag, sender| {
                    let mut on_improve = |obj_value: i128, model: &[bool]| {
                        sender.send_improvement(obj_value, model.to_vec());
                    ...
                    // keep (design §3.2).
                    let _ = solve_optimization_card_descent(
                        instance,
                        objective,
                        timeout_dur,
                        start,
...
/// Solves an optimization PB instance with an explicit worker budget.
///
...
/// reconciliation are unchanged.  Requests above the portfolio's global sane
/// maximum are clamped; a one-worker request takes the sequential path.
pub fn solve_optimization_portfolio_parallel_with_workers(
    instance: &Arc<PbInstance>,
    objective: &PbObjective,
    timeout_dur: Option<Duration>,
    start: Instant,
    ...
    run_parallel_optimization_with_upgrade(
        instance,
        objective,
        timeout_dur,
        start,
        term_flag,
        on_improve,
        workers.get().min(PARALLEL_MAX_WORKERS),
        OptimizationPortfolioRoute::Standard,
        None,
    )
}

/// Parallel optimization with one explicitly supplied external optimum prover.
#[doc(hidden)]
pub fn solve_optimization_portfolio_parallel_with_upgrade(
    instance: &Arc<PbInstance>,
    objective: &PbObjective,
    timeout_dur: Option<Duration>,
    start: Instant,
...
/// Parallel reduced-WBO optimization with an explicit external optimum prover.
#[doc(hidden)]
pub fn solve_wbo_reduced_optimization_portfolio_parallel_with_upgrade(
    instance: &Arc<PbInstance>,
    objective: &PbObjective,
    timeout_dur: Option<Duration>,
    start: Instant,
...
#[allow(clippy::too_many_arguments)]
fn run_parallel_optimization_with_upgrade(
    instance: &Arc<PbInstance>,
    objective: &PbObjective,
    timeout_dur: Option<Duration>,
    start: Instant,
    ...
    run_parallel_optimization_traced_with_upgrade(
        instance,
        objective,
        timeout_dur,
        start,
...
/*
 * Keep the no-upgrade traced entry as the unit-test/diagnostic surface. The
 * injected sibling is used only by the leaf bridge and the routed production
 * call above.
 */
#[allow(clippy::too_many_arguments)]
#[cfg(test)]
...
#[allow(clippy::too_many_arguments)]
fn run_parallel_optimization_traced_with_upgrade(
    instance: &Arc<PbInstance>,
    objective: &PbObjective,
    timeout_dur: Option<Duration>,
    start: Instant,
    ...
    // reactive MEMLIMIT watermark mid-flight.
    let core_budget = clamp_parallel_workers_by_memory(instance, worker_count);
    let mut specs = optimization_worker_specs_with_upgrade(&profile, route, optimum_upgrade);
    let spawn = specs.len().min(core_budget.max(1));
    if spawn <= 1 {
        let solution = match optimum_upgrade {
            Some(upgrade) => solve_optimization_portfolio_with_upgrade(
                instance,
                objective,
                timeout_dur,
...
/// Fixed-cardinality violation descent primal worker
/// (`crate::optimize::card_descent`).
///
/// Applies ONLY to unicost covering programs — `min c * sum x_v` over monotone
/// `>=` rows, every constrained variable priced at the same positive `c` (the
/// full gate is documented on `card_descent::build_cover_view`). Everything
/// else declines in one structural pass, freeing its core for backfill.
///
/// This worker NEVER claims a proven OPTIMUM or UNSAT: it only ever produces
/// strictly-better *feasible* incumbents (reported as `Satisfiable`). Every
/// incumbent is re-verified against ALL original constraints inside the
/// descent (`verify_all_constraints` + exact `eval_objective`) and AGAIN here
/// through `sanitize_optimization_incumbent` — two independent
/// feasibility+objective checks — so an infeasible or mis-valued incumbent is
/// impossible to emit.
fn solve_optimization_card_descent(
    instance: &PbInstance,
    objective: &PbObjective,
    timeout_dur: Option<Duration>,
    start: Instant,
    ...
    // The covering view is exact only for linear PB rows; decline on
    // non-linear inputs (sound: no verdict, no incumbent).
    if !is_linear(instance) {
        return unknown_solution();
    }
    ...
    let mut best_incumbent: Option<(Vec<bool>, i128)> = None;
    {
        let mut card_on_improve = |obj_value: i128, model: &[bool]| {
            // SOUNDNESS GATE (second, independent of the one inside the
            // descent): re-verify against ALL original constraints and
        ...
        let _ = crate::optimize::card_descent::search(
            instance,
            objective,
            deadline,
            &should_stop,
...
fn try_one_row_negative_knapsack_incumbent(
    ...
    if items.len() < ONE_ROW_NEGATIVE_KNAP_MIN_TERMS {
        // FORGONE COST: the items handed back to the general lane by a floor whose
        // claim is that a knapsack this small is not worth the specialised route.
        ay_core::forgone::charge(ay_core::forgone::PB_ONE_ROW_KNAP, items.len() as u64);
        return None;
    }
