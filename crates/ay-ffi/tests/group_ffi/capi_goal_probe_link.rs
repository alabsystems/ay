...
//! Integration test: compile, link and run `tests/capi_goal_probe_consumer.c`
//! against the ay-ffi static library.
//!
//! The consumer exercises the Z3-compatible Goal completion + Probe C API
//! (`Z3_goal_*` / `Z3_probe_*`) on real goals and asserts every value equals the
//! value libz3 4.15.4 returns for the same goal. The SAME source compiled with
...
fn compile_only(header_dir: &Path, c_source: &Path, obj_file: &Path) {
    ...
    assert!(
        status.success(),
        "goal/probe consumer failed to compile against ay_z3_compat.h"
    );
}
...
fn compile_and_link(header_dir: &Path, c_source: &Path, static_lib: &Path, binary: &Path) {
    ...
    assert!(
        ...
        "goal/probe consumer failed to link against libay_ffi.a:\n{}",
        String::from_utf8_lossy(&output.stderr)
    );
}

#[test]
fn test_capi_goal_probe_consumer_compiles_links_runs() {
    let manifest_dir = env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR must be set");
    let crate_root = Path::new(&manifest_dir);
    let c_source = crate_root.join("tests/capi_goal_probe_consumer.c");
    ...
    let tmpdir = env::temp_dir().join("ay_capi_goal_probe_test");
    let _ = std::fs::create_dir_all(&tmpdir);

    let Some(static_lib) = find_static_lib() else {
        eprintln!("SKIP link: libay_ffi.a not found, compile-only check");
        compile_only(&header_dir, &c_source, &tmpdir.join("capi_goal_probe.o"));
        let _ = std::fs::remove_dir_all(&tmpdir);
        return;
    };

    let binary = tmpdir.join("capi_goal_probe_consumer");
    compile_and_link(&header_dir, &c_source, &static_lib, &binary);

    let output = Command::new(&binary)
        .output()
        .expect("failed to run goal/probe consumer binary");
    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        output.status.success(),
        "goal/probe consumer exited with error.\nstdout: {stdout}\nstderr: {stderr}"
    );
    assert!(
        stdout.contains("goal/probe C consumer checks passed"),
        "goal/probe consumer did not report all checks passing.\nstdout: {stdout}"
    );
