...
pub unsafe extern "C" fn Z3_mk_set_add(c: Z3_context, set: Z3_ast, elem: Z3_ast) -> Z3_ast {
    ...
    unsafe {
        ffi_guard_ast(c, |ctx| {
            let tt = ctx.solver.bool_const(true);
            let t = ctx.solver.store(ast_to_term(set), ast_to_term(elem), tt);
            let a = term_to_ast(t);
...
pub unsafe extern "C" fn Z3_mk_set_del(c: Z3_context, set: Z3_ast, elem: Z3_ast) -> Z3_ast {
    ...
    unsafe {
        ffi_guard_ast(c, |ctx| {
            let f = ctx.solver.bool_const(false);
            let t = ctx.solver.store(ast_to_term(set), ast_to_term(elem), f);
            let a = term_to_ast(t);
            if let Some(sort) = lookup_ast_sort(ctx, set).cloned() {
                record_ast_sort(ctx, a, sort);
            }
...
pub unsafe extern "C" fn Z3_mk_set_member(c: Z3_context, elem: Z3_ast, set: Z3_ast) -> Z3_ast {
    ...
    unsafe {
        ffi_guard_ast(c, |ctx| {
            let t = ctx.solver.select(ast_to_term(set), ast_to_term(elem));
            let a = term_to_ast(t);
...
pub unsafe extern "C" fn Z3_mk_set_has_size(c: Z3_context, set: Z3_ast, k: Z3_ast) -> Z3_ast {
    ...
    unsafe {
        ffi_guard_ast(c, |ctx| {
            ...
            let set_t = ast_to_term(set);
            let k_t = ast_to_term(k);
...
pub unsafe extern "C" fn Z3_mk_seq_to_re(c: Z3_context, seq: Z3_ast) -> Z3_ast {
    ...
    // and catches panics so none cross the FFI boundary.
    unsafe {
        ffi_guard_ast(c, |ctx| match ctx.solver.try_str_to_re(ast_to_term(seq)) {
            Ok(t) => {
                let a = term_to_ast(t);
...
pub unsafe extern "C" fn Z3_mk_seq_in_re(c: Z3_context, seq: Z3_ast, re: Z3_ast) -> Z3_ast {
    ...
    unsafe {
        ffi_guard_ast(c, |ctx| {
            match ctx.solver.try_str_in_re(ast_to_term(seq), ast_to_term(re)) {
                Ok(t) => {
                    let a = term_to_ast(t);
                    record_ast_sort(ctx, a, Sort::Bool);
                    a
...
pub unsafe extern "C" fn Z3_mk_re_star(c: Z3_context, re: Z3_ast) -> Z3_ast {
    ...
    // and catches panics so none cross the FFI boundary.
    unsafe {
        ffi_guard_ast(c, |ctx| match ctx.solver.try_re_star(ast_to_term(re)) {
            Ok(t) => {
                let a = term_to_ast(t);
                record_ast_sort(ctx, a, Sort::RegLan);
                a
...
pub unsafe extern "C" fn Z3_mk_re_plus(c: Z3_context, re: Z3_ast) -> Z3_ast {
    ...
    // and catches panics so none cross the FFI boundary.
    unsafe {
        ffi_guard_ast(c, |ctx| match ctx.solver.try_re_plus(ast_to_term(re)) {
            Ok(t) => {
                let a = term_to_ast(t);
                record_ast_sort(ctx, a, Sort::RegLan);
                a
...
pub unsafe extern "C" fn Z3_mk_re_option(c: Z3_context, re: Z3_ast) -> Z3_ast {
    ...
    // and catches panics so none cross the FFI boundary.
    unsafe {
        ffi_guard_ast(c, |ctx| match ctx.solver.try_re_opt(ast_to_term(re)) {
            Ok(t) => {
                let a = term_to_ast(t);
                record_ast_sort(ctx, a, Sort::RegLan);
                a
...
pub unsafe extern "C" fn Z3_mk_re_complement(c: Z3_context, re: Z3_ast) -> Z3_ast {
    ...
    // and catches panics so none cross the FFI boundary.
    unsafe {
        ffi_guard_ast(c, |ctx| match ctx.solver.try_re_comp(ast_to_term(re)) {
            Ok(t) => {
                let a = term_to_ast(t);
                record_ast_sort(ctx, a, Sort::RegLan);
                a
...
pub unsafe extern "C" fn Z3_mk_re_range(c: Z3_context, lo: Z3_ast, hi: Z3_ast) -> Z3_ast {
    ...
    unsafe {
        ffi_guard_ast(c, |ctx| {
            match ctx.solver.try_re_range(ast_to_term(lo), ast_to_term(hi)) {
                Ok(t) => {
                    let a = term_to_ast(t);
                    record_ast_sort(ctx, a, Sort::RegLan);
                    a
...
pub unsafe extern "C" fn Z3_mk_re_loop(
    ...
    unsafe {
        ffi_guard_ast(c, |ctx| {
            match ctx.solver.try_re_loop(ast_to_term(re), lo, hi) {
                Ok(t) => {
                    let a = term_to_ast(t);
                    record_ast_sort(ctx, a, Sort::RegLan);
                    a
