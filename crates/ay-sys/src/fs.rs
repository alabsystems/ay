...
//! Audited Linux filesystem syscall wrappers missing from the pinned safe OS facade.
...
///
/// This provides one Linux `renameat2(RENAME_NOREPLACE)` implementation across
/// libc targets, including those where the pinned `nix` release does not expose
/// its safe wrapper.
