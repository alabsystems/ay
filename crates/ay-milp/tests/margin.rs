...
///
/// # T1: BOTH POSTURES, not just the default one
///
/// `require_certificates` is a FILTER on the finished verdict, not a work
/// switch, so a lane that is correct only with the policy off is a lane that
/// throws a completed answer away. That is exactly what the `MarginMode::Auto`
/// entry used to do: its nested reframed solve INHERITED the caller's policy,
/// and the native tree cannot emit an `OptimalityCertificate` for a model with
/// a real objective — which the reframed model always has — so every INTEGRAL
/// fixture below degraded to `Unknown` under the strict posture while the
/// continuous ones (whose exact rim does export the dual) sailed through.
/// Running only the default posture could not see it.
...
/// An integral band that the ORIGINAL LP RELAXATION CANNOT REFUTE.
///
/// `2x + 2y <= 3` with `x, y` binary admits `x + y <= 1` integrally but
/// `x + y = 1.5` in the relaxation, and the marked band asks for
/// `x + y >= 1.5`. So:
///
/// * the original is INFEASIBLE, but no ROOT Farkas exists for it — the
///   relaxation has the point `(1, 0.5)`;
/// * the reframed `max x + y` is `1 < 1.5`, a strict threshold crossing;
/// * the only third-party-checkable evidence is therefore the whole-tree
///   cover, replayed leaf by leaf against the ORIGINAL model.
///
/// That makes this the fixture that separates the two halves of the fix: the
/// root-crossing enrichment cannot rescue it, so it decides ONLY when the
/// empty-prefix margin-proof dispatch actually delivers the target.
...
/// T1 (cover half). The same both-postures contract on the shape whose only
/// possible evidence is the tree cover.
#[test]
...
/// T6. The ordinary `check()` entry, NO shared prefix, strict policy: the
/// verdict must arrive as the whole-tree certificate arm — the same evidence
/// object `check_marked_margin_shared_binary_prefix` ships — replayed against
/// the ORIGINAL model.
#[test]
...
/// T3 (Auto twin of `insufficient_tree_leaf_cap_fails_closed_after_bound_crossing`).
/// A crossing the capture cannot COVER is not a verdict: no root witness exists
/// for this shape either, so the only honest answer is `Unknown`.
#[test]
...
/// T4 (Auto twin of `equality_at_the_margin_never_triggers_bound_proof`).
/// Equality is not exclusion: a point ON the closed row bound may witness the
/// original, so the bound proof must not fire.
#[test]
fn auto_path_equality_at_the_margin_never_triggers_bound_proof() {
    ...
    let outcome = BabSession::new(
        ...
        &opts()
            ...
            // No nodes: the root bound is exactly the threshold, which is the
            // state under test. With search allowed, `R` itself refutes.
            .with_engine(EngineEconomics::new().with_cuts(false).with_max_nodes(0)),
    )
    .expect("session")
    .check()
...
/// T7. FALSE-UNSAT teeth. A band that IS reachable, marked, under the strict
/// posture: the answer must be SAT and the point must satisfy the ORIGINAL
/// model — the margin row the reframe RELAXED included.
#[test]
...
/// T9. THE ROOT ARM: marking a row must never make ny WEAKER than not marking
/// it.
///
/// `R` (`x >= 1`) is feasible on its own, so the reframe runs a real solve and
/// maps a threshold crossing; with capture disabled there is no tree to cover
/// it. The ORIGINAL model's LP relaxation is nonetheless contradictory
/// (`x >= 1` and `x <= 0`), and that root witness is what the UNMARKED plain
/// solve has always exported. The marked path must export it too.
#[test]
fn auto_path_root_crossing_exports_the_same_root_farkas_as_the_unmarked_solve() {
    ...
    // No tree capture at all: the cover arm cannot fire, so this pins the ROOT
    // arm specifically.
    let solve_opts = opts()
        .with_require_certificates(true)
