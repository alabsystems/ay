...
fn text_command_publication_consumes_private_sat_authority() {
    ...
    assert!(
        unsat_cert.contains("mod query_epoch_access;"),
        "UNSAT certification must compile the query-epoch accessors"
    );
    let unsat_accessors = read("src/executor/unsat_cert/query_epoch_access.rs");
    let unsat_consumer_start = unsat_accessors
    ...
    let unsat_consumer = &unsat_accessors[unsat_consumer_start..];
    assert!(
    ...
    let lifecycle = read("src/executor/lifecycle/unknown_publication.rs");
    ...
    let replace_end = lifecycle[replace_start..]
        .find("pub(crate) fn record_unknown_from_origin(")
        .map(|offset| replace_start + offset)
        .expect("Unknown replacement must precede origin recording");
...
include!("sat_chokepoint_conformance/certificate_lifecycle.rs");
