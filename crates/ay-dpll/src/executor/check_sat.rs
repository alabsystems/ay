...
impl Executor {
    ...
    fn check_sat_guarded(
        ...
        let result = self.cegar_refine_solve_with_finite_model_last_chance()?;
    ...
    pub(in crate::executor) fn cegar_refine_solve(&mut self) -> Result<SolveResult> {
        ...
        self.ctx.assertions = cegar_snapshot.clone();
        let result = self.deep_qe_unknown_retry(result, &cegar_snapshot)?;
        // #quantified-trace-arming runs LAST in the `Unknown` cascade: it is the
        // most expensive fallback (a full second solve with the recorder on) and
        // the cheapest ones should get their chance first.
        self.quantified_trace_arming_unknown_retry(result, &cegar_snapshot)
    }
...
include!("check_sat/conflict_semantic_memo_tests.rs");
