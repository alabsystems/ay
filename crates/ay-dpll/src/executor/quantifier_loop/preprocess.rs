...
include!("preprocess/term_rewrites.rs");
...
impl Executor {
    ...
    /// proof instance. The shared exact walker preserves raw constructors and
    /// capture-safe nested binders, and fails closed on lets, ambiguous binder
    /// names, captured witnesses, or excessive DAG work.
    ...
    /// The universal-quantifier conjuncts of `source`'s positive top-level
    /// `and`-spine, in deterministic spine order.
    ///
    /// Only bare nested `and` applications are descended — never `or`, `not`,
    /// `=>`, or `ite` — so every returned forall is ENTAILED by asserting
    /// `source` (conjunction projection), exactly the set the checked-SAT
    /// builder's `authored_conjunct_closure` accepts an `assume` for.
    fn positive_and_spine_forall_conjuncts(&self, source: TermId) -> Vec<TermId> {
        let mut out = Vec::new();
    ...
    /// The `(= expr CONST)` conjuncts of `source`'s positive `and`-spine, as
    /// (expr, value, conjunct-term) triples — the licensing facts for a
    /// synthesized propagation record. Same spine discipline as
    /// [`Self::positive_and_spine_forall_conjuncts`].
    fn positive_and_spine_value_equalities(&self, source: TermId) -> Vec<(TermId, TermId, TermId)> {
        let mut out = Vec::new();
        let mut stack = vec![source];
        let mut budget = 512usize;
        while let Some(term) = stack.pop() {
            ...
            match self.ctx.terms.get(term) {
                ...
                _ => {
                    if let Some((expr, value)) =
                        crate::preprocess::PropagateValues::extract_value_equality(
                            &self.ctx.terms,
                            term,
                        )
                    {
                        out.push((expr, value, term));
                    }
                }
            }
        }
    ...
    /// Map a (possibly preprocessing-rewritten) instantiated quantifier to the
    /// AUTHORED forall conjunct it descends from (the deductive-checks letleak shape:
    /// `(and (= len 1) (forall i. …len…))` conjunct-split and entailed-consts
    /// rewritten before E-matching instantiates it). Fail-closed on every leg:
    /// no unambiguous singleton source, zero or multiple binding-compatible
    /// forall conjuncts, or a failed exact substitution all decline.
    fn authored_conjunct_forall_for_instantiation(
        &mut self,
        quantifier: TermId,
        binding: &[TermId],
    ) -> Option<TermId> {
        ...
        let direct: HashSet<TermId> = provenance
            ...
            .copied()
            .collect();
        if direct.contains(&quantifier) {
            return None;
        }
        // Direct case: the instantiated quantifier IS a conjunct of a direct
        // authored assertion (E-matching also indexes the original quantified
        // assertions, so the un-rewritten conjunct forall instantiates too).
        let direct_roots: Vec<TermId> = provenance.original_problem_assertions.clone();
        for &source in &direct_roots {
            if self
                .positive_and_spine_forall_conjuncts(source)
                .contains(&quantifier)
            {
                return self
                    .exact_forall_instance(quantifier, binding)
                    .map(|_| quantifier);
            }
        }
        // Rewritten case: the quantifier is a whole derived assertion with one
        // unambiguous authored source whose and-spine holds exactly one
        // binding-compatible forall conjunct.
        let source_sets = provenance.assertion_sources.get(&quantifier)?;
        let [source_set] = source_sets.as_slice() else {
            return None;
        };
        let [source] = source_set.as_slice() else {
        ...
        if !direct.contains(source) {
            return None;
        }
        let source = *source;
        let conjuncts = self.positive_and_spine_forall_conjuncts(source);
        let mut matched = None;
        for conjunct in conjuncts {
            if self.exact_forall_instance(conjunct, binding).is_some() {
                if matched.is_some() {
                    return None;
                }
                matched = Some(conjunct);
            }
        }
        matched
    }
    ...
    pub(super) fn register_ematching_proof_provenance(
        ...
        for record in records {
            if !self.register_ematching_proof_record(
                record,
                &direct_sources,
                &normalized_sources,
                &direct_indices,
                ...
                self.quantified_proof_translation_incomplete = true;
                self.record_authored_conjunct_ematching_provenance(record, &direct_indices);
            }
        }
    }

    fn register_ematching_proof_record(
        &mut self,
        record: &crate::ematching::ForallInstantiationProvenance,
        direct_sources: &HashSet<TermId>,
        normalized_sources: &HashMap<TermId, TermId>,
        direct_indices: &HashMap<TermId, usize>,
    ) -> bool {
        if direct_sources.contains(&record.quantifier) {
            ...
            return registered.is_some();
        }

        let Some(&source) = normalized_sources.get(&record.quantifier) else {
            return false;
        };
        let registered = self
            .proof_tracker
    ...
    /// Narrow authority to an exact authored positive-AND-spine forall. If
    /// preprocessing changed its instance, retain the authored equality
    /// rewrites so downstream proof reconstruction revalidates that change;
    /// the rewritten term never acquires authored authority by resemblance.
    fn record_authored_conjunct_ematching_provenance(
        &mut self,
        record: &crate::ematching::ForallInstantiationProvenance,
        direct_indices: &HashMap<TermId, usize>,
    ) {
        let Some(authored) =
            self.authored_conjunct_forall_for_instantiation(record.quantifier, &record.binding)
        else {
            return;
        };
        let enclosing = direct_indices.iter().find_map(|(&source, &index)| {
            self.positive_and_spine_forall_conjuncts(source)
                .contains(&authored)
                .then_some((source, index))
        });
        let (Some((source, assertion_index)), Some(exact)) = (
            enclosing,
            self.exact_forall_instance(authored, &record.binding),
        ) else {
            return;
        };
        let proof_record = super::super::EmatchingProofRecord {
        ...
        if exact == record.instance {
            return;
        }

        let entries: Vec<_> = self
            .positive_and_spine_value_equalities(source)
            .into_iter()
            .map(
                |(expr, value, conjunct)| crate::preprocess::PropagatedEntrySource {
                    expr,
                    value,
                    source_assertion: conjunct,
                    stamp: 1,
                },
            )
            .collect();
        let already = self
            ...
            .rewrites
            .iter()
            .any(|rewrite| rewrite.before == exact && rewrite.after == record.instance);
        if !entries.is_empty() && !already {
            self.merge_propagation_records(crate::preprocess::PropagationRecords {
                entries,
        ...
        for (idx, ground) in expanded {
                ...
                && crate::quant_unit_authority::quant_unit_authority_enabled()
            {
                ...
                if skolemized.is_some() {
                    ...
                    {
                        if !contains_quantifier(&self.ctx.terms, record.instance) {
