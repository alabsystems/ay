...
use crate::executor::{
    DtContextConflictRecord, Executor, QpfPremiseForcedInstanceRecord, QueryAuthorityEpoch,
    SkolemInstanceRecord,
};
...
impl CheckedContextDerivation {
    ...
    /// (a premise negation already among the clause literals, or repeated),
    ...
    /// Re-verify every stamp and yield the normalized-clause key plus the
    /// fragment payload.
    pub(super) fn into_current(
        self,
        executor: &Executor,
    ) -> Option<(Vec<TermId>, FragmentContextDerivation)> {
        (self
            .query_epoch
            .is_same_epoch(&executor.query_authority_epoch)
            && self.source_context_stamp == executor.ctx.source_context_stamp()
