...
// Textual inclusion preserves `api::SolverConfig` and its public methods.
include!("config.rs");
...
pub struct Solver {
    ...
    /// Function names excluded from definitional-forall macro adoption
    /// ([`Solver::suppress_definitional_adoption`]). An adopted definition
    /// expands every later application of the head symbol, which erases the
    /// ground applications and quantifier bodies an E-matching-only
    /// (`mark_no_mbqi`) axiom needs as triggers — the embedder knows which
    /// heads carry that discipline, the solver cannot infer it at adoption
    /// time (the marked axiom arrives AFTER the definition).
    adoption_suppressed_funs: std::collections::HashSet<String>,
...
impl Solver {
    ...
    fn try_new_with_config_stack_guarded(
        ...
        Ok(Self {
            ...
            adoption_suppressed_funs: std::collections::HashSet::default(),
