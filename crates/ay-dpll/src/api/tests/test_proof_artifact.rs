...
/// Naming a native literal-false assertion for core attribution must not move
/// the contradiction outside the strict proof's authored premise authority.
///
/// `:produce-unsat-cores` routes named assertions through the assumption
/// solver.  This is the exact shape emitted by verification-consumer after simplifying a
/// ground obligation to `false`; the anonymous sibling above exercises the
/// plain solver and therefore does not cover this transport boundary.
#[test]
fn artifact_named_literal_false_core_is_present_and_strict_verified() {
    let mut solver = Solver::new(Logic::All);
    solver.set_produce_proofs(true);
    solver
        .try_set_option(":check-proofs-strict", "true")
        .expect("enable the strict proof checker");
    solver.set_produce_unsat_cores(true);

    let false_term = solver.bool_const(false);
    solver
        .try_assert_named(false_term, "dn0")
        .expect("name literal false");

    let details = solver.check_sat_with_details();
    assert!(
        details.result.is_unsat(),
        "named literal false must remain strictly provable: {:?} / {:?}",
        details.result,
        solver.unknown_reason(),
    );
    details
        .accept_for_consumer()
        .expect("named literal false must carry consumer authority");

    solver
        .last_strict_proof_quality()
        .expect("named literal false must retain its proof")
        .unwrap_or_else(|error| panic!("strict checker rejected named literal false: {error}"));

    let artifact = solver
        .export_last_unsat_artifact()
        .expect("named literal false must publish a proof artifact");
    assert!(artifact.quality.is_complete(), "{}", artifact.quality);
    assert!(matches!(
        artifact.strict_verdict,
    ...
    assert!(
        artifact.alethe.contains("(cl (not false)) :rule false)"),
        "the fixed Alethe false axiom must remain explicit: {}",
        artifact.alethe,
    );
    assert!(
        !artifact.alethe.contains(":rule false :args"),
        "the internal clausification source is not an Alethe false-rule arg: {}",
        artifact.alethe,
    );
    assert_eq!(
        artifact.accept_for_consumer(ProofAcceptanceMode::Strict),
...
fn bundle_export_rechecks_datatype_array_extensionality() {
    ...
    // EUF promotion removes its old hole; array extensionality retains one.
...
fn datatype_equality_chain_distinct_constructors_is_strict_verified() {
    ...
    // The refutation closes on a registry-validated datatype lemma. Two
    // strict-checkable forms are acceptable: the decomposed `dt_distinct`
    // core (+ weakening), or — since the ground refuter gained and/or
    // Boolean semantics (#dt-context-derivation) — ONE
    // `DatatypeGroundConflict` lemma covering the whole and-packed conflict,
    // which the funnel now classifies directly.
    assert!(
        proof.steps.iter().any(|step| matches!(
            step,
            ProofStep::TheoryLemma {
                kind: TheoryLemmaKind::DatatypeDistinct | TheoryLemmaKind::DatatypeGroundConflict,
                ..
            }
        )),
        "the refutation must close on a registry-validated datatype lemma:\n{}",
        artifact.alethe,
    );
    // Under the decomposed form the chain is stated as eq_transitive; the
    // single ground-conflict lemma folds the chain into its own validated
    // clause, so the step is only required in the decomposed shape.
    let decomposed = proof.steps.iter().any(|step| {
        matches!(
            step,
            ProofStep::TheoryLemma {
                kind: TheoryLemmaKind::DatatypeDistinct,
...
include!("test_proof_artifact/store_permutation.rs");
