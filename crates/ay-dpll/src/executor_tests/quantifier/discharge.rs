...
/// with `d > 0` is genuinely UNSAT, but the CE-var bail is retained there, so
/// the honest verdict is unknown — it must never be sat.
#[test]
fn test_cegqi_symbolic_divisor_stays_fail_closed() {
    ...
    let mut exec = Executor::new();
    let outputs = exec.execute_all(&commands).unwrap();
    assert_ne!(outputs, vec!["sat"], "false symbolic-div forall accepted");
}

/// Division by literal ZERO keeps the bail and the #57 semantics:
/// `(= v (div v 0))` is satisfiable (underspecified), never a refutation.
#[test]
