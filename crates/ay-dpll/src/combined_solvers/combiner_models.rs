...
impl TheoryCombiner<'_> {
    ...
    pub(crate) fn extract_all_models_auflia_with_lia_fixup(
        ...
        fixup(self.terms, &mut euf_model, &mut lia_model);
        merge_lia_values(&mut euf_model, lia_model.as_ref());
        // #A1 STALE-MARKER re-adjudication. `function_table_conflicts` is a
        // VERDICT ABOUT A VALUATION — which congruent rows cannot be made to
        // agree — and the pass that reached it above ran on the PRE-fixup
        // snapshot. The re-run just moved row values again and no path
        // retracts a marker, so what would ship downstream is a verdict about
        // a valuation this model no longer has. Its consumer
        // (`complete_unconstrained_constants_for_output`) then discards a
        // model that is valid under the FINAL values, and a genuine `sat`
        // degrades to `unknown (incomplete)` with "No model available". That
        // consumer is RIGHT — given a true marker, discarding is the correct
        // fail-closed action — so the repair belongs at its stale INPUT.
        //
        // Repair before, adjudicate after: drop the provisional markers and
        // re-derive them from the values array extraction will actually see.
        // The clear is an exact retraction, not a loss of evidence:
        // `restore_uf_congruence_under_final_values` is the only site in the
        // workspace that inserts into this set, and `euf_model` was built
        // locally by this function, so everything cleared here was recorded by
        // the call above. Nothing is weakened — same predicate, same guards,
        // and a collision that is still irreparable is marked again, on
        // better evidence.
        euf_model.function_table_conflicts.clear();
        if let Some(lia) = lia_model.as_mut() {
            self.restore_uf_congruence_under_final_values(&mut euf_model, lia, assertions);
        }
        #[cfg(debug_assertions)]
        let lia_value_count = lia_model.as_ref().map_or(0, |m| m.values.len());
        #[cfg(debug_assertions)]
    ...
    fn reunify_lia_values_across_euf_classes(
        ...
        self.restore_uf_congruence_under_final_values(euf_model, lia, assertions);
    }
