...
//! NLSAT-style techniques for NIA: feasible-set look-ahead and arithmetic
//! propagation branching.
//!
//! Ported from NRA's nlsat.rs (#8453) to bring NIA to parity with NRA's
//! clauseSMT capabilities.
//!
...
impl NiaSolver<'_> {
    ...
    /// Evaluate a term to a rational value using the current LRA model (via LIA).
    ...
    /// Called during the NIA check loop to maintain arithmetic propagation state.
    ///
    ...
    pub(crate) fn update_feasible_sets(&mut self) {
        ...
        if self.debug && (!self.blocked_vars.is_empty() || !self.fixed_vars.is_empty()) {
            safe_eprintln!(
                "[NIA] feasible-set update: {} blocked, {} fixed, {} narrowed (total {} vars tracked)",
