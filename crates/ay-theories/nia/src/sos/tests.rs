...
//! Unit tests for the nia-local rational SOS / Positivstellensatz certificate
//! search and the independent checker. Mirrors the NRA checker tests. The
//! checker tests deliberately tamper with valid certificates and require
//! rejection.
...
#[test]
fn search_cross_term_x_minus_y_squared() {
    // (x − y)^2 < 0  ⇔  x² − 2xy + y² < 0 is UNSAT (a square is ≥ 0). This is
    // the target coupled-multivariate NIA shape. The (x − y)^2 dictionary square
    // supplies σ0.
    let x = v(1);
    let y = v(2);
    let c0 = con(poly(&[(&[x, x], 1), (&[x, y], -2), (&[y, y], 1)]), Rel::Lt);
...
fn search_equality_reduced_infeasibility() {
    // { y = 2, x²+y² ≤ 1 }  is UNSAT (y=2 ⇒ y²=4 > 1). Check the search either
    // finds a certificate or cleanly declines (never a bad cert).
    let x = v(1);
    let y = v(2);
