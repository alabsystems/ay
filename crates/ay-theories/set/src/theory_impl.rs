...
//! `TheorySolver` implementation for [`SetSolver`].
//!
//! Implements the DPLL(T) theory interface for the native finite-set theory.
//! The implementation is **fail-closed**: when an out-of-fragment operator is
//! present, [`check`](SetSolver::check) returns [`TheoryResult::Unknown`]
//! rather than risk an unsound SAT/UNSAT verdict.

use super::*;

impl TheorySolver for SetSolver<'_> {
    ...
    fn check(&mut self) -> TheoryResult {
        ...
        // Structural: e ∈ empty is an immediate contradiction.
        if let Some(conflict) = self.check_empty_membership() {
            return TheoryResult::Unsat(conflict);
        }

        // Subset refutation by ground-witness saturation.
    ...
    fn reset(&mut self) {
        ...
        self.trail.clear();
        self.scopes.clear();
        self.member_atoms.clear();
        self.subset_atoms.clear();
        self.card_terms.clear();
    ...
    fn collect_statistics(&self) -> Vec<(&'static str, u64)> {
        vec![
            ("set_member_atoms", self.member_atoms.len() as u64),
            ("set_subset_atoms", self.subset_atoms.len() as u64),
            ("set_card_terms", self.card_terms.len() as u64),
            ("set_empty_terms", self.empty_terms.len() as u64),
            ("set_out_of_fragment", u64::from(self.out_of_fragment)),
        ]
    }
}
