...
//! `TheorySolver` implementation for [`MultisetSolver`].
//!
//! Implements the DPLL(T) theory interface for the native multiset theory.
//! The implementation is **fail-closed**: when an out-of-fragment operator is
//! present, [`check`](MultisetSolver::check) returns
//! [`TheoryResult::Unknown`] rather than risk an unsound SAT/UNSAT verdict.

use super::*;

impl TheorySolver for MultisetSolver<'_> {
    ...
    fn collect_statistics(&self) -> Vec<(&'static str, u64)> {
        vec![
            ("multiset_count_atoms", self.count_atoms.len() as u64),
            ("multiset_subset_atoms", self.subset_atoms.len() as u64),
            ("multiset_empty_terms", self.empty_terms.len() as u64),
            ("multiset_out_of_fragment", u64::from(self.out_of_fragment)),
        ]
    }
}
