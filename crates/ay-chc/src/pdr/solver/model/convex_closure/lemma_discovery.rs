...
//! Candidate validation and admission for convex-closure discoveries.

use super::{ChcVar, FxHashMap, FxHashSet, Lemma, PdrSolver, PredicateId};
use crate::convex_closure::ConvexClosureResult;
use crate::pdr::solver::detect_triangular_pattern;

impl PdrSolver {
    pub(super) fn convex_generalization_lemmas(
        &mut self,
        predicate: PredicateId,
        level: usize,
        numeric_vars: &[ChcVar],
        seen_points: &FxHashSet<Vec<i64>>,
        result: &ConvexClosureResult,
    ) -> Vec<Lemma> {
        ...
        // Convert discovered constraints to lemmas in the original priority order.
        let mut lemmas = Vec::new();
        self.append_convex_equalities(predicate, level, result, &canonical_vars, &mut lemmas);
        self.append_convex_bounds(predicate, level, result, &canonical_vars, &mut lemmas);
        self.append_convex_divisibility(predicate, level, result, &canonical_vars, &mut lemmas);

        self.append_triangular_lemmas(
            predicate,
            level,
            numeric_vars,
            seen_points,
    ...
    fn append_triangular_lemmas(
        ...
        for i in 0..numeric_vars.len() {
            for j in 0..numeric_vars.len() {
                ...
                if let Some(bounds) =
                    detect_triangular_pattern(&data_points, i, j, numeric_vars, self.config.verbose)
                {
                    for bound in bounds {
    ...
    pub(super) fn try_affine_discovery_for_predicate(
        &mut self,
        predicate: PredicateId,
        step_values: &[FxHashMap<String, i64>],
    ) -> bool {
        ...
        if self.learn_affine_equalities(predicate, &result, &canonical_vars) {
            learned_any = true;
        }
        if self.learn_affine_bounds(predicate, &result, &canonical_vars) {
            learned_any = true;
        }
        if self.learn_affine_divisibility(predicate, &result, &canonical_vars) {
            learned_any = true;
        }
        learned_any
    }

    fn learn_affine_equalities(
        &mut self,
        predicate: PredicateId,
        result: &ConvexClosureResult,
        canonical_vars: &[ChcVar],
