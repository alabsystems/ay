...
impl Context {
    ...
    pub(super) fn elaborate_indexed_app(
        ...
        if name == "is" {
            ...
            //
            // #dt-tester-double-elaborate: the fold needs the ARGUMENT'S SORT, so
            // it elaborates `args[0]`, and the delegation on the non-fold path
            // then elaborates the same `args` AGAIN. Elaboration hash-conses, so
            // the second pass returns the same `TermId` — it is pure repeated
            // work, and because testers nest it DOUBLES PER NESTING LEVEL. On the
            // `barrett-jsat` QF_DT files (90 testers over three 2-constructor
            // datatypes) frontend elaboration was ~36% of total solve time, split
            // into two near-equal sibling subtrees per level — the doubling
            // signature.
            //
            // `sole_ctor_exists` is a NECESSARY CONDITION of `is_sole_ctor` that
            // needs no argument sort: the fold can only fire if SOME declared
            // datatype has `ctor_name` as its one and only constructor (compared
            // through `dt_surface_name`, exactly as `is_sole_ctor` compares). When
            // no such datatype is declared the fold provably cannot fire, so
            // skipping the speculative elaboration changes nothing about which
            // term is produced. Every SMT-LIB datatype with two or more
            // constructors — i.e. every datatype for which a tester carries any
            // information at all — takes this path.
            let sole_ctor_exists = self.datatypes.values().any(|ctors| {
                ctors.len() == 1
                    && self.dt_surface_name(&ctors[0]).unwrap_or(&ctors[0]) == ctor_name
            });
            if args.len() == 1 && sole_ctor_exists {
...
include!("indexed/pb_parameters.rs");
