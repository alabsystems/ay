...
pub(crate) struct ProofManager {
    ...
    next_lrat_id: u64,
    /// Inclusive end of the original-clause ID block reserved when the LRAT
    /// writer was constructed. Originals inside this block must retain their
    /// canonical IDs even if a derived step is emitted before all of them are
    /// ingested (for example, vivification before the final input clause).
    reserved_original_clause_ids: u64,
    #[cfg(debug_assertions)]
...
impl ProofManager {
    ...
    /// Inclusive end of the LRAT writer's construction-time original-ID
    /// reservation. A zero value means there is no reserved original block.
    #[inline]
    pub(crate) fn reserved_original_clause_ids(&self) -> u64 {
        self.reserved_original_clause_ids
    }
