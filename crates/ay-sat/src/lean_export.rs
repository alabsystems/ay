...
//!
//! 2. **Kernel-checked** ([`write_lean4_lrat_kernel`]): emits a fully
...
//!    Lean rejects the file with exit code 1. This is the form ay emits
//!    toward #8697 (sound proof generation for lean-smt).
//!
...
/// Write a kernel-checkable Lean4 LRAT proof (#8697 Phase 1).
///
...
///
/// The Lean4 kernel will reject the file (exit code 1) if `lratCheck`
/// evaluates to `false` on the emitted data, meaning the proof is
/// actually checked — not merely parsed.
///
...
/// per-problem `lratCheck … = true` by pure `decide` (kernel reduction; the
/// native compiler is NOT in the TCB). This is the production form whose
...
mod tests {
    ...
    fn test_lean4_verified_emits_grounded_shape() {
        ...
        // The verified emitter must import the machine-checked checker and ground
        // the verdict in `lratCheck_sound` (NOT re-derive an unverified checker).
        assert!(
        ...
        // It must NOT re-define an unverified lratCheck or use native_decide for the
        // soundness step (the per-problem check is pure `decide`).
        assert!(
