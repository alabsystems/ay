// Copyright 2026 Andrew Yates
// Author: Andrew Yates
// Licensed under the Apache License, Version 2.0

//! Wire-lowering tests for `AletheRule::FreshDefBound`.
//!
//! The step is certified INTERNALLY (`ay-proof`'s `FreshDefRegistry` re-derives
//! freshness from the problem's own symbols) and is deliberately UNCHECKABLE
//! externally: Alethe has no notion of a solver introducing a symbol and
//! defining it. So it renders as an honest `hole` — byte-for-byte what the
//! premiseless `trust` it replaces already rendered as, which is what makes the
//! promotion lane a no-op on the emitted document.
//!
//! Two near-misses these tests exist to prevent:
//!
//! 1. `hole :args (__ay_eqdv!1)`. The pinned carcara build rejects `:args` on
//!    `hole` outright, which takes the whole document from `holey` to
//!    `invalid` — strictly worse than the step it replaced. The default
//!    generic-step renderer WOULD print the args, so the dedicated arm is
//!    load-bearing rather than cosmetic.
//! 2. A real rule name. `la_generic` would claim `(<= d lin)` is a rational
//!    tautology; it is FALSE at `d = lin + 1`. Claiming it would be a false
//!    certificate, not a weaker one.

use super::*;
use ay_core::{AletheRule, Sort, TermStore};
use num_bigint::BigInt;

/// `d := x - y`, the shape `EqDiffVar` builds for `(= x y)`.
fn definitional_bound(terms: &mut TermStore, lower: bool) -> (ProofStep, TermId) {
    let d = terms.mk_var("__ay_eqdv!1", Sort::Int);
    let x = terms.mk_var("x", Sort::Int);
    let y = terms.mk_var("y", Sort::Int);
    let neg_y = terms.mk_neg(y);
    let lin = terms.mk_add(vec![x, neg_y]);
    let atom = if lower {
        terms.mk_le(lin, d)
    } else {
        terms.mk_le(d, lin)
    };
    (
        ProofStep::Step {
            rule: AletheRule::FreshDefBound,
            clause: vec![atom],
            premises: Vec::new(),
            args: vec![d],
        },
        d,
    )
}

#[test]
fn a_fresh_def_bound_lowers_to_an_honest_hole_with_no_args() {
    let mut terms = TermStore::new();
    let (step, _) = definitional_bound(&mut terms, false);

    let rendered = AlethePrinter::new(&terms)
        .format_step(&step, ProofId(7))
        .expect("a well-formed fresh-definition bound must render, not error");

    assert_eq!(
        rendered,
        "(step t7 (cl (<= __ay_eqdv!1 (+ x (- y)))) :rule hole)"
    );
    assert!(!rendered.contains(":args"), "{rendered}");
    assert!(!rendered.contains(":premises"), "{rendered}");
    assert!(!rendered.contains("la_generic"), "{rendered}");
    assert!(!rendered.contains("fresh_def_bound"), "{rendered}");
}

#[test]
fn the_lower_bound_direction_lowers_the_same_way() {
    let mut terms = TermStore::new();
    let (step, _) = definitional_bound(&mut terms, true);
    let rendered = AlethePrinter::new(&terms)
        .format_step(&step, ProofId(0))
        .expect("the lower bound must render too");
    assert_eq!(
        rendered,
        "(step t0 (cl (<= (+ x (- y)) __ay_eqdv!1)) :rule hole)"
    );
    assert!(!rendered.contains(":args"), "{rendered}");
}

#[test]
fn the_promotion_is_byte_identical_to_the_trust_step_it_replaces() {
    // The whole lane is an INTERNAL certification change. If the emitted
    // document differed, the promotion could regress an external verdict; this
    // pins that it cannot.
    let mut terms = TermStore::new();
    let (promoted, _) = definitional_bound(&mut terms, false);
    let ProofStep::Step { clause, .. } = &promoted else {
        panic!("built a Step");
    };
    let demoted = ProofStep::Step {
        rule: AletheRule::Trust,
        clause: clause.clone(),
        premises: Vec::new(),
        args: Vec::new(),
    };
    let printer = AlethePrinter::new(&terms);
    let promoted_text = printer
        .format_step(&promoted, ProofId(3))
        .expect("promoted step renders");
    let demoted_text = printer
        .format_step(&demoted, ProofId(3))
        .expect("demoted step renders");
    assert_eq!(promoted_text, demoted_text);
}

#[test]
fn a_malformed_fresh_def_bound_declines_instead_of_reaching_the_wire() {
    // The printer emits this step's CLAUSE, so a step whose `:args` name a
    // symbol that is on NEITHER side of the `<=` must not be printed as though
    // it were a definition. `(<= x y)` is an ordinary constraint, falsified at
    // `x = 1, y = 0`.
    let mut terms = TermStore::new();
    let d = terms.mk_var("__ay_eqdv!1", Sort::Int);
    let x = terms.mk_var("x", Sort::Int);
    let y = terms.mk_var("y", Sort::Int);
    let atom = terms.mk_le(x, y);
    let step = ProofStep::Step {
        rule: AletheRule::FreshDefBound,
        clause: vec![atom],
        premises: Vec::new(),
        args: vec![d],
    };
    let error = AlethePrinter::new(&terms)
        .format_step(&step, ProofId(0))
        .expect_err("a malformed definition must decline");
    assert!(
        matches!(error, AlethePrintError::InvalidSurfaceStep { .. }),
        "{error:?}"
    );
}

#[test]
fn a_multi_literal_fresh_def_bound_declines() {
    // A wider clause is a disjunction and is not a definition; printing it as
    // one `hole` would understate nothing, but the step is malformed and the
    // fail-closed default must be a decline rather than a silent pass-through
    // to the generic renderer (which would print `:args`).
    let mut terms = TermStore::new();
    let d = terms.mk_var("__ay_eqdv!1", Sort::Int);
    let x = terms.mk_var("x", Sort::Int);
    let zero = terms.mk_int(BigInt::from(0));
    let first = terms.mk_le(d, x);
    let second = terms.mk_le(zero, x);
    let step = ProofStep::Step {
        rule: AletheRule::FreshDefBound,
        clause: vec![first, second],
        premises: Vec::new(),
        args: vec![d],
    };
    assert!(AlethePrinter::new(&terms)
        .format_step(&step, ProofId(0))
        .is_err());
}
