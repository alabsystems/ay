...
mod tests {
    ...
    fn shift_shape_declines_malformed_comparison_and_extract_sorts() {
        ...
        let TermData::App(_, shifted_args) = bad_extract.terms.get(bad_extract.shifted).clone()
        else {
            unreachable!()
        };
        ...
        let TermData::App(extract_op, extract_args) = bad_extract.terms.get(*extract).clone()
        else {
            unreachable!()
        };
        ...
        let TermData::App(_, non_strict_args) =
            bad_extract.terms.get(bad_extract.non_strict).clone()
        else {
            unreachable!()
        };
