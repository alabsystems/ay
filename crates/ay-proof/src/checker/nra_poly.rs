...
impl MPoly {
    ...
    pub(crate) fn as_constant(&self) -> Option<BigRational> {
    ...
    pub(crate) fn collect_vars(&self, out: &mut BTreeSet<TermId>) {
    ...
    fn add_monomial(&mut self, m: Monomial, c: BigRational) {
        if c.is_zero() {
            return;
        }
    ...
    pub(crate) fn add_assign_from(
        ...
        meter.charge_ops(other.terms.len() as u64 + 1)?;
        meter.charge_monomials(other.terms.len())?;
        for (m, c) in &other.terms {
            self.add_monomial(m.clone(), c.clone());
        }
        Ok(())
    }
    ...
    pub(crate) fn sub(&self, other: &Self, meter: &mut WorkMeter) -> Result<Self, String> {
        meter.charge_ops(self.terms.len() as u64)?;
        let mut out = self.clone();
        out.sub_assign_from(other, meter)?;
        Ok(out)
    }

    pub(crate) fn scale(&self, c: &BigRational, meter: &mut WorkMeter) -> Result<Self, String> {
        meter.charge_ops(self.terms.len() as u64)?;
        if c.is_zero() {
            return Ok(Self::zero());
        }
    ...
    pub(crate) fn mul(&self, other: &Self, meter: &mut WorkMeter) -> Result<Self, String> {
        let product_size = self.terms.len().saturating_mul(other.terms.len());
        let bits = max_coeff_bits(self.terms.values()).max(max_coeff_bits(other.terms.values()));
...
pub(crate) fn extract_constraints(
    ...
    // of every literal.
    let mut pending: Vec<(TermId, bool)> = clause.iter().rev().map(|&l| (l, false)).collect();
    ...
    while let Some((t, asserted)) = pending.pop() {
        meter.charge_node()?;
        match terms.get(t) {
            TermData::Not(inner) => pending.push((*inner, !asserted)),
                ...
                if args.len() == 2 && matches!(name.as_str(), "<" | "<=" | ">" | ">=" | "=") =>
            {
                ...
                let pa = parse_poly(terms, a, &mut memo, meter, 0)?;
                let pb = parse_poly(terms, b, &mut memo, meter, 0)?;
                // Nonlinearity is measured on the parsed SIDES: a polynomial
                // identity refutation (e.g. `(x+y)^2 != x^2+2xy+y^2`) has
                // nonlinear sides whose DIFFERENCE cancels to a constant, and
                // it is a genuinely nonlinear conflict, not a linear-lane one.
                // deg(a - b) <= max(deg a, deg b), so checking the sides
                // subsumes checking the difference; a truly linear conflict
                // (all sides degree <= 1) is still refused.
                if pa.max_total_degree() >= 2 || pb.max_total_degree() >= 2 {
                    has_nonlinear = true;
                }
                let poly = pa.sub(&pb, meter)?;
                let rel = match (name.as_str(), asserted) {
                    ("<", true) => Rel::Lt,
                    ("<", false) => Rel::Ge,
                    ("<=", true) => Rel::Le,
                    ("<=", false) => Rel::Gt,
                ...
                if let Some(c) = poly.as_constant() {
                    // Constant conjunct: evaluate it outright.
                    if !rel.satisfied_by_sign(c.cmp(&BigRational::zero())) {
                        const_refuted = true;
                    }
                } else {
                    constraints.push(Constraint { poly, rel });
                }
            }
            _ => {
                return Err(
...
fn parse_poly(
    ...
    meter.charge_node()?;
    let poly = match terms.get(t) {
        TermData::Const(Constant::Int(n)) => MPoly::constant(BigRational::from(n.clone())),
        TermData::Const(Constant::Rational(r)) => MPoly::constant(r.0.clone()),
        TermData::Var(_, _) if matches!(terms.sort(t), Sort::Int | Sort::Real) => MPoly::var(t),
        ...
        TermData::App(Symbol::Named(name), args) if name == "-" && args.len() == 1 => {
            parse_poly(terms, args[0], memo, meter, depth + 1)?.neg()
        }
            ...
            if name == "/" && args.len() == 2 && matches!(terms.sort(t), Sort::Real) =>
        {
            ...
            // constant (exact scaling); otherwise the whole node is an opaque
            // leaf (universal abstraction, sound for refutation).
            ...
            match denom.as_constant() {
                Some(c) if !c.is_zero() => {
                    ...
                    numer.scale(&(BigRational::one() / c), meter)?
                }
                _ => MPoly::var(t),
            }
        }
    ...
    if poly.max_total_degree() > MAX_POLY_DEGREE {
        return Err(format!(
            "nra polynomial degree exceeds cap {MAX_POLY_DEGREE}"
        ));
    }
...
impl Ival {
    ...
    pub(crate) fn add(&self, other: &Self, meter: &mut WorkMeter) -> Result<Self, String> {
    ...
    pub(crate) fn pow(&self, k: u32, meter: &mut WorkMeter) -> Result<Self, String> {
    ...
    pub(crate) fn inv(&self, meter: &mut WorkMeter) -> Result<Option<Self>, String> {
