...
//!    [`ay_pb_core::verify_all_constraints`], and its exact objective value via
...
use ay_pb_core::portfolio::{LinearOptimumUpgrade, LinearOptimumUpgradeResult};
use ay_pb_core::{eval_objective, verify_all_constraints, PbInstance, PbObjective, PbRel, PbTerm};
...
pub(crate) struct AyMilpUpgrade;

pub(crate) static AY_MILP_UPGRADE: AyMilpUpgrade = AyMilpUpgrade;

impl LinearOptimumUpgrade for AyMilpUpgrade {
    fn eligible(&self, instance: &PbInstance, objective: &PbObjective) -> bool {
        milp_lane_eligible(instance, objective)
    }
    ...
    fn try_optimum_upgrade(
        ...
        let (assignment, value) =
            try_milp_optimum_upgrade(instance, objective, incumbent, budget, on_improve)?;
        Some(LinearOptimumUpgradeResult { assignment, value })
    }
}
...
fn exact_f64(v: i128) -> Option<f64> {
    if v.checked_abs()? > MAX_EXACT_F64 {
        return None;
    }
    Some(v as f64)
}
...
pub(crate) fn milp_lane_eligible(instance: &PbInstance, objective: &PbObjective) -> bool {
    ...
    let exact_magnitude = |value: i128| {
        value
            .checked_abs()
            .is_some_and(|magnitude| magnitude <= MAX_EXACT_F64)
    };
    let term_ok = |t: &PbTerm| t.lits.len() == 1 && exact_magnitude(t.coeff);
    ...
    instance
        .constraints
        .iter()
        .all(|c| exact_magnitude(c.rhs) && c.terms.iter().all(term_ok))
}
...
mod tests {
    use super::*;
    use ay_pb_core::{PbConstraint, PbLit};
    ...
    #[test]
    fn public_portfolio_facade_injects_the_milp_upgrade() {
        let instance = PbInstance {
            num_vars: 2,
            num_constraints: 1,
            constraints: vec![ge(vec![term(2, 1), term(2, 2)], 3)],
        ...
        let term_flag = std::sync::atomic::AtomicBool::new(false);
        let result = crate::portfolio::solve_optimization_portfolio(
            &instance,
            objective,
            Some(Duration::from_secs(10)),
            std::time::Instant::now(),
            &term_flag,
            &mut |_value, _assignment| {},
        );
        assert_eq!(result.status, crate::PbStatus::OptimumFound);
    ...
    #[test]
    fn milp_lane_declines_minimum_i128_without_overflowing_magnitude_checks() {
        let instance = PbInstance {
            num_vars: 1,
            num_constraints: 1,
            constraints: vec![ge(vec![term(i128::MIN, 1)], i128::MIN)],
            objective: Some(PbObjective {
                terms: vec![term(1, 1)],
            }),
        };
        let objective = instance.objective.as_ref().expect("objective");
        assert!(!milp_lane_eligible(&instance, objective));
        assert!(build_model(&instance, objective).is_none());
    }
}
