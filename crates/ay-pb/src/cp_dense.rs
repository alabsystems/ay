...
impl DenseCp {
    ...
    where
        F: FnMut(PbLit) -> bool,
    {
        self.resolve_proven_round_to_one_impl(reason, pivot, falsified_fn, None)
    }
        ...
        F: FnMut(PbLit) -> bool,
    {
        ...
        self.resolve_proven_round_to_one_impl(reason, pivot, falsified_fn, Some(capture))
    }
        ...
        F: FnMut(PbLit) -> bool,
    {
        ...
        let mut reduced = asserting_reason.clone();
        ...
        // `normalize`. We accumulate into a clone of the conflict.
        let mut resolvent = conflict.clone();
        resolvent.add_scaled(&reduced, w)?;
...
mod tests {
    ...
    fn proven_round_to_one_semantic_entailment() {
        ...
        for seed in 0..40_000u64 {
            ...
            let resolvent = match dc.resolve_proven_round_to_one(&dr, pivot, falsified_fn) {
                Ok(r) => r,
                // Err here is invalid-pivot or overflow; both are sound
                // fall-back-to-heuristic signals, not soundness failures.
                Err(_) => continue,
            };
            let cprime = resolvent.to_cp_constraint();
