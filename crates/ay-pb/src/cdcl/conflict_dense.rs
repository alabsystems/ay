...
impl PbCdclSolver {
    ...
    /// never ship a non-implied lemma.
    fn dense_resolve_proven(
        ...
        let result = match capture {
            Some(cap) => {
                learned.resolve_proven_round_to_one_captured(reason, pivot, falsified_fn, cap)
            }
            None => learned.resolve_proven_round_to_one(reason, pivot, falsified_fn),
        };
        match result {
            Ok(resolvent) => {
                *scratch = resolvent;
