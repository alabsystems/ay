    ...
    F: FnMut() -> bool,
{
    ...
    let first_trimmed = trim_core(solver, first_core, should_stop);
    ...
    F: FnMut() -> bool,
{
    ...
    if crate::optimize::lp_bound::cert_path_enabled() {
        if let Some((bound, _cert, outcome)) = crate::optimize::lp_bound::lp_lower_bound_with_cert(
            objective,
            &strengthened.constraints,
            strengthened.num_vars,
            &lp_should_stop,
        ) {
            // `Verified` means the checked certificate corroborates `bound`; in all
            // outcomes `bound` equals the exact-simplex base bound. The cut loop in
            // `lp_lower_bound` can only raise it, so we still defer to that for the
            // tightest floor and use the certified base bound as a sound fallback.
            let _ = outcome;
            return Some(
