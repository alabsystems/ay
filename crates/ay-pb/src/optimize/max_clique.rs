    ...
    F: FnMut() -> bool,
{
    ...
    for (constraint_index, constraint) in instance.constraints.iter().enumerate() {
        ...
        let (lhs, rhs) = detect_binary_conflict(constraint, &objective_index)?;
        conflicts.insert(normalized_pair(lhs, rhs));
    }
...
fn validate_assignment(
    ...
    Some(eval_objective(objective, assignment))
}
