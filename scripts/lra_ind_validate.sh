#!/usr/bin/env bash
# ay-script: lra-ind-validate
# File-aligned soundness + coverage validation for QF_LRA .ind files.
# Usage: lra_ind_validate.sh <ay_binary> <timeout_s> <label> [env-prefix...]
# Compares AY answers (one per line) to pre-generated z3 oracle at /tmp/lra_val/z3_<base>.txt
# Reports per-file definite count and ANY conflict (both definite and differ).
set -u
...
for f in "$DIR"/*.ind_k100.smt2; do
  ...
  # File-aligned conflict check: read both line by line
  conf=0
  cdetail=""
  i=0
  while IFS= read -r ayline; do
    i=$((i+1))
    z3line=$(sed -n "${i}p" "$z3file")
    if { [ "$ayline" = "sat" ] || [ "$ayline" = "unsat" ]; } && \
       { [ "$z3line" = "sat" ] || [ "$z3line" = "unsat" ]; } && \
       [ "$ayline" != "$z3line" ]; then
      conf=$((conf+1))
      cdetail="$cdetail line$i:AY=$ayline,z3=$z3line"
    fi
  done < "$out"
