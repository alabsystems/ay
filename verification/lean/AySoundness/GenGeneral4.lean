...
-/
namespace AySoundness.Emitted.General_c09840ed7ec7c619
open AySoundness
...
def atomVal (m : Val) (n : Nat) : Bool :=
  ...
  | 2 => decide ((m.1 1) ≤ (m.1 0))
  | 3 => decide (((m.2 0) (m.1 0)) = ((m.2 0) (m.1 1)))
  | 4 => decide ((m.1 0) = (m.1 1))
  | _ => false
...
theorem lemma_4_valid (m : Val) : clauseSat (atomVal m) [-1, -2, 4] = true := by
  ...
  by_cases h1 : (m.1 0) ≤ (m.1 1) <;> by_cases h2 : (m.1 1) ≤ (m.1 0) <;> by_cases h4 : (m.1 0) = (m.1 1) <;> simp [h1, h2, h4] <;> omega

theorem lemma_5_valid (m : Val) : clauseSat (atomVal m) [-4, 3] = true := by
  simp only [clauseSat, atomVal, litSat, List.any_cons, List.any_nil]
  by_cases h4 : (m.1 0) = (m.1 1) <;> simp [h4]
...
end AySoundness.Emitted.General_c09840ed7ec7c619
