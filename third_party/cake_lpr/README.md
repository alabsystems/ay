...
> **Vendored copy note (AY):** this directory is a partial snapshot of the
> upstream cake_lpr release (https://github.com/tanyongkiam/cake_lpr). Only
> the ARMv8 assembly `cake_lpr_arm8.S` is vendored, and `make` builds it into
> the `cake_lpr` binary that `ay check` can use. The x64 `cake_lpr.S` is
> **not** vendored — fetch it from upstream for the x64 and starexec targets.
> `cake_lpr.sha256` pins the vendored files. The rest of this README is
> upstream text.
