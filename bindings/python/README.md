...
- **Finite sets (sets-as-arrays, z3py's own encoding):** `SetSort`, `EmptySet`, `FullSet`, `SetAdd`, `SetDel`, `IsMember`, and the algebra ops `SetUnion`, `SetIntersect`, `SetDifference`, `SetComplement` (lazy, membership-reducible). `IsSubset` is **deferred for this array-backed encoding** (it needs quantified array reasoning AY cannot soundly decide).
- **Native finite sets (the Z3 5.0 `FiniteSet` sort):** `FiniteSetSort`, `FiniteSetEmpty`, `Singleton`, `FiniteSetUnion`, `FiniteSetIntersect`, `FiniteSetDifference`, `FiniteSetMember`, `FiniteSetSize`, `FiniteSetSubset`, `FiniteSetMap`, `FiniteSetFilter`, `FiniteSetRange`. `EmptySet(FiniteSetSort(E))` dispatches here, matching Z3 5.0, and `IsSubset` on a `FiniteSetRef` dispatches to `FiniteSetSubset`.
...
- **Set `IsSubset` is deferred on the array-backed encoding.** There it requires quantified array reasoning (`forall x. a[x] => b[x]`), so `IsSubset` raises `NotImplementedError`; on a native `FiniteSetRef` it dispatches to `FiniteSetSubset` instead. Direct quantified-array formulas return `unknown` at the wrapper boundary. The other set operations listed above reduce to quantifier-free membership formulas.
