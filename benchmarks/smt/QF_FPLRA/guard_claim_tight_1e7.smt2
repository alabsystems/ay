; Tight-claim variant of guard_claim_signed_distance.smt2: assert the
; rounding error reaches 1e-7. TRUE status is sat — real f64 rounding errors
; at 2^48 magnitudes reach ~0.15 (concrete witnesses exist), far above 1e-7.
; The forward-error tactic's certified bound is 13/64, which does NOT rule
; this claim out, so the tactic must abstain; the refinement lane cannot find
; the model, so ay must answer a *sound unknown* here — never unsat.
(set-info :status sat)
(set-logic QF_FPLRA)
...
(define-fun rreal () Real (+ (* (fp.to_real nx) (fp.to_real px))
                             ...
                             (fp.to_real d)))
; The bound 13/64 cannot refute error >= 1e-7 (and it is genuinely reachable).
(assert (>= (- (fp.to_real rf) rreal) 0.0000001))
(check-sat)
